...
foreach mode in nacional sectorial {
	forvalues año = 2014(1)2014 {
		foreach mes in 02 05 08 11 {
			...
			* Mutación I:
			generate nocupados = 1
			rc_ene_rama1,       año(`año') mes(`mes')  // rama de actividad
			rc_ene_tramo_ntrab, año(`año') mes(`mes')  // tamaño empresa
			if ("`mode'" == "nacional") {
				replace rama1_rc = 99
			}
			...
			* Agregación I:
			...
			collapse (sum) nocupados [pw = fact], by(`bylist')
...
recode nocupados (. = 0)
collapse nocupados, by(año `bylist')
save `df1', replace
...
foreach mode in nacional sectorial {
	forvalues año = 2014(1)2014 {
		...
		* Agregación:
		collapse (sum) nempresas, by(rama1_rc tramo_ntrab)
...
label variable nocupados "n ocupados"
label variable nempresas "n empresas"

* Guardado:
save "$project/data/chart 03-01-01.dta", replace
beep
